<main class="main_content">
	<div class="container main_account">
		<nav class="main_account_sidebar">
			...
			<a class="radius transition" href="<?= "{$configBase}/conta/meus-dados"; ?>" title="">Meus dados</a>
			...
			<?php for($order = 1; $order < 4; $order++): ?>
				<article class="main_account_content_item">
					<header class="main_account_content_item_header">
						...
						<h1>Pedido: #7876468M<?= $order; ?></h1>
					</header>

					<ul class="main_account_content_item_pdts radius">
						<?php for($img = 1; $img < 8; $img++): ?>
							<li><img src="<?= $configBase; ?>/Uploads/products/0<?= $img; ?>.jpg" /></li>
						<?php endfor; ?>
					</ul>

					<div class="main_account_content_item_desc">
						<p>Aprovado</p>
						<p>R$ 277,00 em 10X no cartão</p>
						<a href="#" title="">ver detalhes</a>
						<a href="#" title="">acompanhar envio</a>
					</div>
				</article>
			<?php endfor; ?>
		</div>
